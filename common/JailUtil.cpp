...
namespace JailUtil
{
bool modaodfwebmount(const std::string& arg, std::string source, std::string target)
{
...
bool bind(const std::string& source, const std::string& target)
{
    ...
    const bool res = modaodfwebmount("-b", source, target);
    if (res)
...
bool remountReadonly(const std::string& source, const std::string& target)
{
    ...
    Poco::File(target).createDirectory();
    const bool res = modaodfwebmount("-r", source, target);
    if (res)
...
static bool unmount(const std::string& target)
{
    ...
    const bool res = modaodfwebmount("-u", "", target);
    if (res)
...
// modaodfweb, and they may have bind-mounting enabled, but the
...
void cleanupJails(const std::string& root)
{
    ...
    else
    {
        ...
        for (const auto& jail : jails)
        {
            ...
            // it gets unmounted by modaodfwebmount during shutdown, but modaodfwebmount
    ...
    if (bindMount)
    {
        ...
        if (bind(sysTemplate, target))
        {
            ...
            LOG_INF("Enabling Bind-Mounting of jail contents for better performance per "
                    "mount_jail_tree config in modaodfweb.xml.");
        }
        else
            LOG_ERR("Bind-Mounting fails and will be disabled for this run. To disable permanently "
                    "set mount_jail_tree config entry in modaodfweb.xml to false.");
    }
    else
...
void setupJailDevNodes(const std::string& root)
{
    ...
    if (!Poco::File(root + "/dev/random").exists())
    {
         const bool res = modaodfwebmount("-d", "", root + "/dev");
         if (res)
