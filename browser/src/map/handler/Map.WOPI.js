...
L.Map.WOPI = L.Handler.extend({
	...
	_postMessageListener: function(e) {
		...
		if (!this._appLoaded) {
			window.app.console.error('MODA ODF WEB not loaded yet. Listen for App_LoadingStatus (Document_Loaded) event before using PostMessage API. Ignoring post message \'' + msg.MessageId + '\'.');
			return;
		}
