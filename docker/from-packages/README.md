...
The following sections explains how to build the container image of MODAODFWEB:
...
1. Run the following command to build the container image:
    ...
    ```bash
    docker build -t modaodfweb:3.8-latest .
    ```
   ...
   **NOTE:** Depending on your Docker installation you may need to run this (and all other Docker) command(s) _as root_.
   ...
   The command output should be similar to the following:
    ...
    => => naming to docker.io/library/modaodfweb:3.8-latest                              0.0s
    ```

   and the built container image should be available locally under the `modaodfweb:3.8-latest` image reference string.
...
1. Run the following commands to launch a temporary container for fetching the original service configuration files:
    ...
    docker run "${docker_run_opts[@]}" modaodfweb:3.8-latest
    ```
...
1. Run the following commands to have a copy of the service configuration files in the directory hosting this document:
    ...
    docker cp "${docker_cp_opts[@]}" modaodfweb-3.8:/etc/modaodfweb/ config
    ```

1. Run the following commands to stop and destroy the temporary container:

    ```bash
    docker container stop modaodfweb-3.8
    ```
...
1. Run the following commands _as root_ to copy your native installation's configuration directory to the directory that hosts this document:
    ...
    cp "${cp_opts[@]}" /etc/modaodfweb config
    ```
...
1. Run the following commands to have a copy of the reference service configuration files in the directory hosting this document:
    ...
    docker cp "${docker_cp_opts[@]}" modaodfweb-3.8:/etc/modaodfweb/ config.reference
    ```
...
1. Run the following commands to directly create and run a container from the built container image:
    ...
    docker_run_opts=(
        ...
        --mount type=bind,source=./config,destination=/etc/modaodfweb
...
Run the following commands to start the MODAODFWEB service:

```bash
docker start modaodfweb-3.8
```
...
```bash
docker compose start modaodfweb
```

### Stop the service

Run the following commands to stop the MODAODFWEB service:

```bash
docker stop modaodfweb-3.8
```

If you're using Docker Compose you can also use this command after changing the working directory to the directory hosting this document:

```bash
docker compose stop modaodfweb
```

### Check the service logs

Run the following commands to check the MODAODFWEB service logs:
...
docker logs "${docker_logs_opts[@]}" modaodfweb-3.8
```
...
```bash
docker container rm modaodfweb-3.8
```
...
```bash
docker compose down modaodfweb
```
