...
public:
                            ...
                            const StringVector& tokens) override
    {
        ...
        if (tokens.equals(0, "getList"))
        {
            ...
            json.stringify(oss);
            sendAdminTextFrame(sendTextMessage, "macipList " + oss.str());
        }
        // 新增來源
        else if (tokens.equals(0, "addSource") && tokens.size() == 3)
        {
            ...
            try
            {
                ...
                std::ostringstream oss;
                json->stringify(oss);
                sendAdminTextFrame(sendTextMessage, cmd + " " + oss.str());

            }
            catch(const Poco::Exception& exc)
            {
                LOG_ERR("Admin module [" << getDetail().name << "]:" << exc.displayText());
                sendAdminTextFrame(sendTextMessage, "Error:" + exc.displayText());
            }
        }
        // 更新來源
        else if (tokens.equals(0, "updateSource") && tokens.size() == 2)
        {
            ...
            try
            {
                ...
                std::ostringstream oss;
                json->stringify(oss);
                sendAdminTextFrame(sendTextMessage, "updateSource " + oss.str());
            }
            catch(const Poco::Exception& exc)
            {
                LOG_ERR("Admin module [" << getDetail().name << "]:" << exc.displayText());
        ...
        else if (tokens.equals(0, "deleteSource") && tokens.size() == 2)
        {
            ...
            try
            {
                ...
                sendAdminTextFrame(sendTextMessage, "deleteSource " + tokens[1]);
            }
            catch(const Poco::Exception& exc)
            {
                LOG_ERR("Admin module [" << getDetail().name << "]:" << exc.displayText());
