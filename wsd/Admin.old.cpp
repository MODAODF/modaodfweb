                                       ...
                                       std::vector<char> &payload)
{
    ...
    else if (tokens[0] == "setConfig" && tokens.count() > 1)
    {
        ...
        if (JsonUtil::parseJSON(firstLine, object))
        {
            ...
            removeSpecialKeys(config, "net.cors_allow.host");
    ...
    else if (tokens[0] == "get_signature")
    {
        std::string signaturePath = LOOLWSD::Product.libDir() + "/signature";
        std::cout << signaturePath << "\n";
        size_t position = 0;
        std::ifstream file(signaturePath, std::ios::binary);
        if (file.is_open())
        {
            ...
            Poco::Base64Encoder b64oss(oss);
            b64oss << retStr;
            b64oss.close();
            std::string result = tokens[0] + " " + oss.str();
            file.close();
            sendTextFrame(result);
        }
        else
        {
            sendTextFrame("getSignatureError");
        }
    }
    else if (tokens[0] == "get_license_info")
    {
        std::string result = " {";
        result += "\"custno\":\"" + LOOLWSD::Product.custNo() + "\",";
        result += "\"custname\":\"" + LOOLWSD::Product.custName()  + "\",";
        result += "\"expiredate\":\"" + LOOLWSD::Product.expiryDateYYYYMMDD() + "\",";
        result += "\"productlist\":" + LOOLWSD::Product.getProductList();
        result += "}";
        result = tokens[0] + result;
        sendTextFrame(result);
    }
    else if (tokens[0] == "send_authorize")
    {
        std::stringstream inoss;
        std::stringstream onoss;
        inoss << tokens[1];
        Poco::Base64Decoder decoder(inoss);
        std::ofstream authorize_file;
        authorize_file.open(LOOLWSD::Product.libDir() + "/authorizer");
        authorize_file << decoder.rdbuf();
        authorize_file.close();

        // Reinitialize
        LOOLWSD::Product.initialize(LOOLWSD::Product.confDir(), LOOLWSD::Product.libDir());
        if (LOOLWSD::Product.isSupported())
        {
            std::string result = " ";
            result += LOOLWSD::Product.custNo() + ",";
            result += LOOLWSD::Product.custName()  + ",";
            result += LOOLWSD::Product.expiryDateYYYYMMDD();
            sendTextFrame("get_license_info " + result);
            sendTextFrame("authorize_result true");
        }
        else
            sendTextFrame("authorize_result false");
    }
