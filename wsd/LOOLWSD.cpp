...
void LOOLWSD::innerInitialize(Application& self)
{
    ...
    // 設定 modaodfweb 所在路徑為也是其他應用程式路徑
    // Set the path of modaodfweb to be the path of other applications.
...
//            "storage.ssl.enable" - deliberately not set; for back-compat
    ...
    LOG_INF("Initializing modaodfweb server [" << ServerName << "]. Experimental features are "
...
int LOOLWSD::main(const std::vector<std::string>& /*args*/)
{
    ...
    LOG_INF("Process [modaodfweb] finished with exit status: " << returnValue);
    return returnValue;
}
