                          ...
                          std::string desc)
{
    ...
    {
        LOG_ERR("Error update mac/ip data: " << e.displayText());
        insert.reset(session);
        session.close();
        return false;
    }
...
bool TemplateRepoDB::macIpReachLimit(std::string ftype)
{
    ...
    {
        LOG_ERR("log file read error: " << e.displayText());
        select.reset(session);
        session.close();
        return true;
    }
...
bool TemplateRepoDB::validateIP(std::string clientAddress)
{
    ...
    if(count > 0)
    {
        std::cout << "Allow " << clientAddress << std::endl;
    }
    else
    {
        std::cout << "Do not allow" << clientAddress << std::endl;
    }
    select.reset(session);
    session.close();
    return count > 0;
