...
bool TemplateRepo::isSameIP(const std::string &host, const std::string &client)
{
    const int tokenOpts = StringTokenizer::TOK_IGNORE_EMPTY |
                          StringTokenizer::TOK_TRIM;
    StringTokenizer hostToken(host, ":", tokenOpts);
    std::string hostIP = hostToken[0];
    StringTokenizer clientToken(client, ":", tokenOpts);
    std::string clientIP;
    if (clientToken.count() >= 2)
    {
        clientIP = clientToken[1];
    }
    else
    {
        clientIP = clientToken[0];
    }
    if (hostIP == clientIP)
    {
        return true;
...
bool TemplateRepo::isLocalhost(const std::string &targetHost)
{
...
bool TemplateRepo::checkIP(std::string host, std::string clientAddress)
{
    TemplateRepoDB *tr_db = new TemplateRepoDB();
    if (!isSameIP(host, clientAddress) &&
        !isLocalhost(clientAddress) &&
        !tr_db->validateIP(clientAddress))
    {
        return false;
    }
    return true;
                              ...
                              HTTPRequest &request)
{
    ...
    if (!checkIP(request.getHost(), socket->clientAddress()))
    {
...
void TemplateRepo::downloadAllTemplates(std::weak_ptr<StreamSocket> _socket)
{
    ...
    HttpHelper::sendFileAndShutdown(socket, zipFilePath, mimeType, &response);
}
...
#if ENABLE_DEBUG
    template_dir = std::string(DEV_DIR) + "/runTimeData/templaterepo/";
#else
